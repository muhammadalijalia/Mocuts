...
public class ProgressHUD: UIView {
	...
	private var colorStatus		= UIColor.clear
    private var colorAnimation	= Theme.appOrangeColor
	private var colorProgress	= UIColor.clear
	...
	private func setupBackground(_ interaction: Bool) {
        ...
        viewBackground?.backgroundColor = .black.withAlphaComponent(0.6)
	...
	private func setupAnimation() {
		...
		if (viewAnimation == nil) {
			viewAnimation = UIView(frame: CGRect(x: 0, y: 0, width: 80, height: 80))
		}
        ...
        if (staticImageView == nil) {
            ...
            staticImageView = UIImageView(frame: CGRect(x: 0, y: 0, width: 75, height: 75))
            staticImageView?.isUserInteractionEnabled = false
            staticImageView?.image = UIImage(named: "Mocuts.Circular")
            
        }
        toolbarHUD?.isUserInteractionEnabled = false
        if (staticImageView?.superview == nil) {
            toolbarHUD?.addSubview(staticImageView!)
            
            staticImageView?.center = self.center
            staticImageView?.layer.shadowColor = UIColor.black.withAlphaComponent(0.6).cgColor
            staticImageView?.layer.shadowOffset = .zero
            staticImageView?.layer.shadowOpacity = 1
            staticImageView?.backgroundColor = .clear
            
            self.bringSubviewToFront(staticImageView!)
//            self.sendSubviewToBack(staticImageView)
        }
