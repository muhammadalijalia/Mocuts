...
class SettingView: BaseView, Routeable {
...
extension SettingView : UITableViewDelegate , UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if indexPath.row == 0 {
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: "SettingListingCell", for: indexPath) as! SettingListingCell
            cell.itemTitle.text = "Delete Account"
            return cell
        }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if indexPath.row == 0 {
            ...
            let vc: DeleteAccountViewController = AppRouter.instantiateViewController(storyboard: .more)
            self.route(to: vc, navigation: .push)
        }
    }
}

extension SettingView : SettingPushNotificationCellDelegate {
