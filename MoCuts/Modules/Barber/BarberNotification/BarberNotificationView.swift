...
class BarberNotificationView: BaseView, Routeable {
    ...
    override func viewDidLoad() {
        ...
        viewModel = BarberNotificationViewModel()
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if !hasLoadedOnce {
            DispatchQueue.main.async {
                ...
                (self.viewModel as! BarberNotificationViewModel).getNotifications()
            }
        }
    }
    ...
    func setupViewModelObserver() {
        if let viewModel = viewModel as? BarberNotificationViewModel {
...
extension BarberNotificationView : UITableViewDelegate , UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        if notifications.count == 0 && !viewModel.isLoading && self.hasLoadedOnce {
    ...
    func tableView(_ tableView: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        if indexPath.row == notifications.count - 1 {
            if notifications.count < total {
                DispatchQueue.main.async {
                    self.removeNoDataLabelOnTableView(tableView: self.tableView)
                    (self.viewModel as! BarberNotificationViewModel).getNotifications(offset: (self.page + 1), showLoader: true)
                }
            }
        }
    }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if indexPath.section == 1 {
            ...
            if notifications[indexPath.row].readAt ?? "" == "" {
                ...
                (viewModel as! BarberNotificationViewModel).markRead(notificationId: notificationId)
            }
...
//                 Constants.NotificationType.CUSTOMER_REVIEWED,
...
extension BarberNotificationView {
