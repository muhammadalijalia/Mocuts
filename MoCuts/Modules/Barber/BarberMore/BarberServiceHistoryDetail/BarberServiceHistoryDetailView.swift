//
//  BarberServiceHistoryDetailView.swift
//  MoCuts
//
...
class BarberServiceHistoryDetailView: BaseView, Routeable {
    ...
    func setView() {
        ...
        coverImage.sd_setImage(with: URL(string: jobDetails?.user?.image_url ?? ""), completed: nil)
        nameLabel.text = jobDetails?.user?.name
        let rating = jobDetails?.barber?.average_rating ?? 0.0
        ratingLabel.text = String(format: rating == 0 ? "%.1f" : "%.2f", rating)
        
        addressLabel.text = jobDetails?.user?.address?.decodeUrl()?.replacingOccurrences(of: "+", with: " ")
            ...
            alert.addAction(UIAlertAction(title: reportType.name, style: .default, handler: { action in
                ...
                for reportType in self.reportTypes {
                    if reportType.name == action.title {
                        ...
                        (self.viewModel as! BarberServiceRequestViewModel).sendReport(toId: String(self.jobDetails?.user_id ?? 0), reportTypeId: String(reportType.id ?? 0), message: "Reported")
                    }
                }
            }))
        }
...
extension BarberServiceHistoryDetailView : UITableViewDelegate , UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.row == 0 {
            ...
            let commision = Double(self.jobDetails?.commission ?? 0)
            ...
            cell.totalCharges.text = "$ \(String(format: "%.2f", (subTotal - ((commision/100.0) * subTotal))))"
            return cell
        } else {
...
extension BarberServiceHistoryDetailView {
