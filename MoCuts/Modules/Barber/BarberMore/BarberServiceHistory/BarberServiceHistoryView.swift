//
//  BarberServiceHistoryView.swift
//  MoCuts
//
...
class BarberServiceHistoryView: BaseView, Routeable {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        viewModel = BarberServiceHistoryViewModel()
    ...
    func getDataFromVM(offset: Int = 1) {
        ...
        (viewModel as! BarberServiceHistoryViewModel).getServiceCompletedJobs(barberID: "\(self.barberId ?? 0)", month: params.month, year: params.year, query: params.query, status: params.status, offset: params.page, isFiltered: params.isFiltered)
    }
    ...
    func setJobsLists() {
        (viewModel as! BarberServiceHistoryViewModel).setJobsRoute = { [weak self] jobModel in
        ...
        (viewModel as! BarberServiceHistoryViewModel).setFailureRoute = { [weak self] error in
            DispatchQueue.main.async {
                guard let self = self else {
                    return
                }
    ...
    func setView() {
        ...
        let filterIcon = UIImage(named: "filterWhite")
        let rightBarBtn = UIBarButtonItem(image: filterIcon, style: .plain, target: self, action: #selector(filterBtnTapped))
        if let image = backButtonImage {
            let backButton = self.backBarButton(image: image)
            Helper.getInstance.setNavigationBar(leftBarItem: backButton, rightBarItem: rightBarBtn, title: "Services History", isTransparent: false, isBottomLine: false, titleView: nil, backgroundColor: Theme.appNavigationBlueColor, itemsColor: .white, titleFontStyle: UIFont(name: Constants.mediumFont, size: 20), vc: self)
        }
    ...
    @objc func filterBtnTapped() {
        ...
        alert.addAction(UIAlertAction(title: "User Cancelled", style: .default, handler: { _ in
    ...
    func setTableView() {
        ...
        tableView.register(UINib(nibName: "BarberServiceHistoryCell", bundle: nil), forCellReuseIdentifier: "BarberServiceHistoryCell")
...
//            monthYearPickerView?.preferredDatePickerStyle = .wheels
        ...
        filterTextfield.accessibilityIdentifier = "datePicker"
...
extension BarberServiceHistoryView : UITableViewDelegate , UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "BarberServiceHistoryCell", for: indexPath) as! BarberServiceHistoryCell
        cell.bgView.backgroundColor = UIColor(hex: "#EDF8FF")
        cell.shopName.text = jobsArray[indexPath.row].user?.name
        cell.statusBtn.setImage(nil, for: .normal)
        cell.statusBtn.setTitle(getStatus(with: jobsArray[indexPath.row].status ?? 30), for: .normal)
        cell.serviceName.text = jobsArray[indexPath.row].job_services?.first?.service?.name
        ...
        cell.shopImage?.sd_setImage(with: URL(string: jobsArray[indexPath.row].user?.image_url ?? ""), completed: nil)
        return cell
    }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        let vc : BarberServiceHistoryDetailView = AppRouter.instantiateViewController(storyboard: .Barbermore)
...
extension BarberServiceHistoryView : UITextFieldDelegate {
...
extension BarberServiceHistoryView {
